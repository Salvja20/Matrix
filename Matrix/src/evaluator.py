    ...
    def define(self, name, value):
        ...
        if name in self.vars:
            raise ImmutabilityError(
                ...
                f"Matrix variables are immutable."
            )
        self.vars[name] = value
...
class Evaluator:
    ...
    def eval_arithmetic(self, op, left, right):
        ...
        if lt != rt:
            raise TypeError_(
                f"Cannot apply '{op}' to '{lt}' and '{rt}'. "
                f"Matrix enforces strict typing — use int() or float() to convert explicitly."
            )
        ...
        if lt != rt:
            if not (left is None and right is None):
                raise TypeError_(
                    ...
                    f"Matrix requires same-type comparisons."
                )
